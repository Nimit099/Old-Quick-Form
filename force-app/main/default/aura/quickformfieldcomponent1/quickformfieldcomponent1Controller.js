({
    ...
    // fire event after check Full Name field data is null
    blankValueCheckFullName: function(component, event, helper) {
        try {
            var fieldData = event.getSource().get("v.value");
            if (fieldData == '') {
                var fId = component.get('v.fId');
                ...
                action.setCallback(this, function(response) {
                    ...
                    if (fieldId != null) {
                        var fieldLabel = 'Last Name';
                        var target = [];
                        target.push(fieldId, fieldLabel, fieldData, requiredField);
                        var evt = $A.get("e.c:PreviewEvent");
                        evt.setParams({ "formFieldData": target });
                        evt.fire();
                    }
